...
func TestBuilder_IndexOf_fulltree(t *testing.T) {
	...
	for i, c := range cases {
		bb := NewBuilder(c.bitmapSize)
		got := bb.IndexOf(k, c.frombit)
		gots := fmt.Sprintf("%032b", got)
		ta.Equal(c.idx, gots, "%d-th: case: %+v", i+1, c)
	}
}

func TestBuilder_IndexOf_partialtree(t *testing.T) {
...
func TestBuilder_IndexesOf(t *testing.T) {

	ta := require.New(t)

	k := "abcdefgh"
	bb := NewBuilder(1<<22 - 1)

	got := bb.IndexesOf([]string{k, k}, 3, false)
	ta.Equal(2, len(got))
	...
	got = bb.IndexesOf([]string{k, k}, 3, true)
	ta.Equal(1, len(got))
	ta.Equal("0000000000000000000000000000000000000000000000101100010011010011", fmt.Sprintf("%064b", got[0]))
}
